...
class Game:
    ...
    def show_sidebar(self, surface):
        ...
        for i in range(0, len(self.board.move_history), 2):
            ...
            # White's move
            white = self.board.move_history[i]

            # Black's move (if it exists)
            black = ""

            if i + 1 < len(self.board.move_history):
                black = self.board.move_history[i + 1]

            # Format and render the move number with both moves
            text = font.render(
            ...
            # Draw the move text onto the sidebar
